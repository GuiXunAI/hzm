...
const App: React.FC = () => {
  ...
  const addGuardian = () => {
    if (editGuardians.length < 3) {
      setEditGuardians([...editGuardians, { 
  ...
  const removeGuardian = (id: string) => {
    if (editGuardians.length > 1) {
      setEditGuardians(editGuardians.filter(g => g.id !== id));
    }
  };

  const hasCheckedInToday = state.checkInHistory.some(h => h.dateString === new Date().toLocaleDateString());

  const countdownToAlert = useMemo(() => {
    if (!state.lastCheckIn) return 48;
    const diff = Date.now() - state.lastCheckIn;
    const remaining = Math.max(0, 48 - (diff / (1000 * 60 * 60)));
  ...
  if (!state.isRegistered) return <Onboarding onComplete={(lang, name, email) => {
    ...
    setState(p => ({ 
      ...p, 
      language: lang, 
      userContact: initialUser, 
      emergencyContacts: initialGuardians, 
      isRegistered: true 
    }));
    setEditUser(initialUser);
    setEditGuardians(initialGuardians);
  }} />;
      ...
      <main className="flex-1 overflow-y-auto no-scrollbar relative pt-14 pb-32 px-7">
        ...
        {activeTab === 'home' && (
          <div className="flex flex-col gap-6 animate-in fade-in slide-in-from-bottom-4 duration-500 mt-4">
            <div className={`premium-card p-6 border-2 transition-all duration-500 ${hasCheckedInToday ? 'border-[#EFFFF4]' : countdownToAlert < 12 ? 'border-orange-100 bg-orange-50/30' : 'border-[#FF4D4F] bg-[#FFF1F0]'}`}>
              <div className="flex items-center justify-between mb-4">
                 ...
                 {!hasCheckedInToday && (
                   <span className={`text-[11px] font-bold px-3 py-1 rounded-full ${countdownToAlert < 12 ? 'bg-orange-500 text-white animate-pulse' : 'bg-rose-500/10 text-rose-500'}`}>
                     {state.language === 'en' ? `Alert in ${countdownToAlert}h` : `距离预警仅剩 ${countdownToAlert}h`}
                   </span>
                 )}
              </div>
            ...
            <div className="bg-[#F8F9FA] rounded-[24px] p-6 text-center border border-slate-100">
               <p className="text-[13px] text-slate-400 font-medium leading-relaxed">
                  {state.language === 'en' ? 'Daily check-ins ensure your safety. Alerts are sent to emergency contacts after 48h silence.' : '若超过48小时未确认平安，系统将自动向紧急联系人发送预警邮件，请保持每日签到习惯。'}
               </p>
            </div>
          </div>
        )}
